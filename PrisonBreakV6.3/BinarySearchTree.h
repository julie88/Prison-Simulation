                                        ...
                                        string getName(ItemType *item))
{
	...
	rootPtr = _insert(rootPtr, newNodePtr,getName);
	return true;
}
                                        ...
                                        string getID(ItemType *item))
{
	...
	rootPtr = _remove(rootPtr, target, isSuccessful,getID);
	return isSuccessful;
}
    ...
    (ItemType& anEntry, string getName(ItemType *item)) const
{
	...
	//CALL
	found = findNode(rootPtr, anEntry, getName);
	while (found) {
...
void BinarySearchTree<ItemType>::levelTraverse(void visit(ItemType&)) {
	...
	//INIT
	if (rootPtr) coreQueue.enqueue(rootPtr);
